...
function(hipblaslt_create_device_library)
    ...
    set(_multi ARCHES PYTHONPATH_EXTRA RUNTIME_LIB_DIRS TOOL_BIN_DIRS)
    ...
    if(NOT HIPBLASLT_PYTHON_COMMAND)
        if(NOT Python3_EXECUTABLE)
            message(FATAL_ERROR
                "hipblaslt_create_device_library: HIPBLASLT_PYTHON_COMMAND is not set and "
                "Python3_EXECUTABLE is unavailable; call find_package(Python3) before this.")
        endif()
        if(WIN32)
            set(_ds "$<SEMICOLON>")
        else()
        ...
        set(_cdl_env "")
        if(_cdl_PYTHONPATH_EXTRA)
            list(JOIN _cdl_PYTHONPATH_EXTRA "${_ds}" _cdl_pp)
            list(APPEND _cdl_env "PYTHONPATH=${_cdl_pp}")
        endif()
        if(WIN32)
            set(_cdl_path "$ENV{PATH}")
            string(REPLACE ";" "${_ds}" _cdl_path "${_cdl_path}")
            set(_cdl_win_dirs ${_cdl_TOOL_BIN_DIRS} ${_cdl_RUNTIME_LIB_DIRS})
            if(_cdl_win_dirs)
                list(JOIN _cdl_win_dirs "${_ds}" _cdl_win_dirs)
                list(APPEND _cdl_env "PATH=${_cdl_win_dirs}${_ds}${_cdl_path}")
            else()
                list(APPEND _cdl_env "PATH=${_cdl_path}")
            endif()
        else()
            if(_cdl_RUNTIME_LIB_DIRS)
                list(JOIN _cdl_RUNTIME_LIB_DIRS ":" _cdl_ld)
                list(APPEND _cdl_env "LD_LIBRARY_PATH=${_cdl_ld}:$ENV{LD_LIBRARY_PATH}")
            endif()
            if(_cdl_TOOL_BIN_DIRS)
                list(JOIN _cdl_TOOL_BIN_DIRS ":" _cdl_pb)
                list(APPEND _cdl_env "PATH=${_cdl_pb}:$ENV{PATH}")
            endif()
        endif()
        set(HIPBLASLT_PYTHON_COMMAND
            "${CMAKE_COMMAND}" -E env ${_cdl_env} -- "${Python3_EXECUTABLE}")
    endif()
